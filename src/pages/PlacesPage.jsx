...
export default function PlacesPage() {
  ...
  const categories = useMemo(() => {
    ...
    return [t('placesPage.filters.all', 'Tất cả'), ...Array.from(cats)];
  }, [lang, t]);

  // Reset active category when language changes to avoid mismatch
  useEffect(() => {
    setActiveCategory(t('placesPage.filters.all', 'Tất cả'));
  }, [lang, t]);

  // Map category to icon from preferenceOptions
  const getCategoryConfig = (catName) => {
    if (catName === t('placesPage.filters.all', 'Tất cả')) return { icon: '📍', color: 'from-primary-500 to-primary-700' };
    const match = preferenceOptions.find(p => (p.label[lang] || p.label.vi || p.label) === catName);
  ...
  const filteredPlaces = useMemo(() => {
    ...
    // Filter by category
    if (activeCategory !== t('placesPage.filters.all', 'Tất cả')) {
  ...
  return (
    <>
      <SEO 
        ...
        description="Danh sách các địa điểm du lịch hấp dẫn, quán ăn ngon và quán cafe check-in tuyệt đẹp tại Gia Lai." 
      />
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 sm:py-20 lg:py-24">
          <div className="text-center animate-fade-in-up">
            ...
            <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-white/10 backdrop-blur-md border border-white/20 text-white/90 text-sm font-medium mb-6">
              ...
              {t('placesPage.hero.explore', { count: totalPlaces })}
            </div>
          ...
          <Suspense fallback={
            ...
            <div className="h-[500px] lg:h-[600px]">
              <MapView
                ...
                type="place"
                onItemClick={(item) => navigate(`/place-detail/${item.id}`)}
              />
            </div>
          </Suspense>
        ) : isLoading ? (
